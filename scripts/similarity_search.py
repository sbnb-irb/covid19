...
def main(simtype):
    ...
    df_lit = pd.DataFrame(R, columns = ["inchikey", "name", "evidence", "moa", "descriptions", "links"])
    fn = "outputs/df_lit_%s.pkl" % simtype
    ...
    iks_lit = np.array([k for k in sorted(df_lit["inchikey"]) if k in iks_can_set])
    ...
    with h5py.File("outputs/dist_%s.h5" % simtype, "w") as hf:
    ...
    def similarities_as_matrix(ranks, min_evidence, moa):
        ...
        M = np.vstack([cou_tp5, cou_pv5, cou_pv4, cou_pv3, cou_pv2, best_idx]).astype(np.int)
        M = M.T
        return M
    ...
    def similarities(simtype, min_evidence, moa, sort_by="lpv_4"):
        ...
        df.to_pickle(os.path.join("outputs", fn))
        return fn
    ...
    for min_evidence in [None, 0, 1, 2, 3]:
        for moa in [None, 0, 1, 2, 3, 4, 5]:
            print("Sim type %s | Min evidence %s | MoA %s" % (simtype, str(min_evidence), str(moa)))
            legend[(min_evidence, moa)] = similarities(simtype=simtype, min_evidence=min_evidence, moa=moa)
    with open("outputs/legend_%s.pkl" % simtype, "wb") as f:
